...
class PcCompany():
    ...
    def parse(self, name):
        if self.browser.current_url.find('tianyancha') < 0:
            ...
            ele = self.browser.find_element_by_xpath("//input[@id='home-main-search']")
            ele.clear()
            ele.send_keys(name)
            self.browser.find_element_by_xpath("//div[@class='input-group-addon search_button']").click()
        else:
            ele = self.browser.find_element_by_xpath("//input[@id='header-company-search']")
            ele.clear()
            ele.send_keys(name)
            self.browser.find_element_by_xpath("//div[@class='input-group search_group head-search-group-company']/div").click()

        try:
            self.wait.until(lambda broswer: self.browser.find_element_by_xpath("//div[@class='container company_container']/div[1]"))
        except Exception, e:
        ...
        handle = self.browser.find_element_by_xpath("//div[@class='b-c-white search_result_container']/div[1]//a")
        ac = ActionChains(self.browser)
        ac.move_to_element(handle).click().perform()
        if len(self.browser.window_handles) == 1:
            time.sleep(5)
    ...
    def parse_item(self):
        item = {}
        field_and_xpath = {
            'tianyancha_contacter': "//div[@class='holder-outer']/a/div/div/div[@class='new-c3 f18 pt10']",
            'tianyancha_company_name': "//div[@class='company_header_width ie9Style']/div/span",
            'web_url': "//div[@class='company_header_width ie9Style']/div/div/div[@class='in-block vertical-top overflow-width mr20']/a",
            'phone': "//div[@class='company_header_width ie9Style']/div/div/div[@class='in-block vertical-top overflow-width mr20']/span[2]"
        }
...
if __name__ == '__main__':
    ...
    while True:
        try:
            if False: #datetime.datetime.now().hour < 8 or datetime.datetime.now().hour >= 20:
